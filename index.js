...
app.post('/audio-transcribe', uploadAudio.single('file'), async (req, res) => {
  ...
  const endpoint = 'https://senpaiapp2.openai.azure.com/openai/deployments/gpt-4o-transcribe/audio/transcriptions?api-version=2025-03-01-preview';
          ...
          const contentTasks = runMessageDatum.content.map(async (item) => {
            if (item.type === "text" && item.text?.value) {
              ...
              if (!fs.existsSync(downloadsDir)) {
                ...
                const fileUrl = `https://senpaiapp2.openai.azure.com/openai/files/${fileId}/content?api-version=2024-05-01-preview`;
                ...
                try {
                  ...
                  const downloadLink = process.env.AZURE === 'true'
                    ? `https://ihisenpai2-epabadgjb5hmdra4.swedencentral-01.azurewebsites.net/downloads/${path.basename(filePath)}`
              ...
              try {
                ...
                const imageResponse = await fetch(`https://senpaiapp2.openai.azure.com/openai/files/${item.image_file.file_id}/content?api-version=2024-05-01-preview`, {
                  headers: {
                    'api-key': process.env.AZURE_OPENAI_KEY
                  }
                });
